# 🤖 Herbia — Serviço de Inteligência Artificial

Serviço de classificação de doenças em plantas usando **MobileNetV2** com PyTorch, exposto via **FastAPI**.

Este serviço é chamado internamente pelo servidor Node.js e não deve ser exposto diretamente ao utilizador final.
...
> ⚠️ O PyTorch será instalado na versão CPU. O download pode demorar alguns minutos.
...
Se `"modelo"` retornar `"Erro"`, verifica se o ficheiro `model/best_model.pth` existe na pasta correta.

---

## 🔗 Integração com o Backend

O servidor Node.js chama este serviço via `POST /predict` enviando a imagem da planta. Certifica-te de que o `ia_service` está a correr **antes** de iniciar o backend.
...
| Threshold de confiança | 50% — abaixo disso retorna `"Desconhecido"` |
...
**Resposta quando a confiança está abaixo do threshold:**
```json
{
  "doenca": "Desconhecido",
