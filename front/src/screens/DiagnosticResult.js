...
export default function DiagnosticResultScreen({ navigation, route }) {
  ...
  const isDesconhecido = estado === 'N/A' || doenca === 'Desconhecido';
    ...
    <SafeAreaView style={[styles.safeContainer, { backgroundColor: currentTheme.background }]} edges={['top']}>
      ...
      >
        ...
        <View style={styles.titleSection}>
          <View style={{ flex: 1 }}>
            {!isSaudavel && !isDesconhecido && (
              <Text style={[styles.foundText, { color: '#FF5252' }]}>
                Doença Detectada
              </Text>
            )}
            
            {/* ✅ Mostra "Planta Saudável" se for saudável */}
            {isSaudavel && (
              <Text style={[styles.foundText, { color: activeColor }]}>
                Planta Saudável
              </Text>
            )}

            {/* ✅ Se for desconhecido, podemos mostrar um aviso neutro ou deixar vazio */}
            {isDesconhecido && (
              <Text style={[styles.foundText, { color: '#888888' }]}>
