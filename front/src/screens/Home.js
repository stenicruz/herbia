...
const CACHE_CULTURAS = '@Herbia:cache_culturas';
...
const loadDica = async () => {
  ...
  const loadCulturas = async () => {
    try {
      ...
      await AsyncStorage.setItem(CACHE_CULTURAS, JSON.stringify(response.data));
    } catch (err) {
      const cached = await AsyncStorage.getItem(CACHE_CULTURAS);
      if (cached) setCulturas(JSON.parse(cached));
...
culturasResumo.map((item) => (
  <View key={item.id} style={styles.culturaItem}>
    <View style={[styles.circuloCultura, {
      ...
      {item.imagem_url ? (
        <Image
          // A key forçar o componente a remontar se a URL mudar
          key={item.imagem_url} 
          source={{ uri: item.imagem_url }}
          style={styles.culturaImage}
          resizeMode="cover"
          // Logs de debug para ver no terminal do VS Code
          onLoad={() => console.log(`Imagem carregada: ${item.nome}`)}
          onError={(e) => console.warn(`Erro na imagem ${item.nome}:`, e.nativeEvent.error)}
        />
      ) : (
